...
export function InviteUser({ onCreated }: InviteUserProps) {
  ...
  return (
    <form className="grid gap-3 rounded-lg border border-line/20 bg-panel/80 p-4 backdrop-blur-xl lg:grid-cols-6">
      <input
        value={name}
        onChange={(event) => setName(event.target.value)}
        className="rounded-md border border-line/20 bg-ink px-3 py-2 text-sm"
      ...
      {status && <p className="text-sm text-muted lg:col-span-6">{status}</p>}
    </form>
  );
}
