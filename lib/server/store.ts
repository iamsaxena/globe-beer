...
export async function listUsers() {
  const remote = await supabaseRequest<ManualUserRecord[]>("manual_users?select=id,name,email,mobile,username,profile_pic,role,disabled,created_at,updated_at&order=created_at.desc");
...
export async function updateManualUserById(id: string, patch: Partial<Pick<ManualUserRecord, "name" | "email" | "mobile" | "username" | "role" | "disabled">> & { password?: string }) {
  if (id === defaultAdmin.id) {
    throw new Error("Built-in admin cannot be edited from Manual Users.");
  }
  const updated_at = new Date().toISOString();
  const dbPatch = {
    ...patch,
  ...
  const remote = await supabaseRequest<ManualUserRecord[]>(`manual_users?id=eq.${encodeURIComponent(id)}`, {
    method: "PATCH",
    body: JSON.stringify(dbPatch)
  });
...
export async function deleteManualUserById(id: string) {
  if (id === defaultAdmin.id) {
    throw new Error("Built-in admin cannot be deleted.");
  }
  const remote = await supabaseRequest<ManualUserRecord[]>(`manual_users?id=eq.${encodeURIComponent(id)}`, { method: "DELETE" });
